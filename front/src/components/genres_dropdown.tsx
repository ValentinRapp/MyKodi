export function GenresDropdown(props: { genres: string[], selectedGenres: string[], setSelectedGenres: (genres: string[]) => void }) {
  ...
  const filterSVG = (
    <svg fill="#000000" height="17px" width="17px" version="1.1" id="Layer_1" xmlns="http://www.w3.org/2000/svg"
  ...
  return (
    <details className="dropdown">
      ...
      <ul
        className="menu dropdown-content bg-base-300 rounded-box z-[1] w-52 p-2 shadow mt-2"
        style={{
          ...
          <li>
            <label>
              <input
                type="checkbox"
                className="checkbox"
                checked={props.selectedGenres.length === props.genres.length}
                onChange={e =>
